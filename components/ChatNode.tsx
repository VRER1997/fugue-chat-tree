...
export const ChatNode = ({ id, data, isConnectable }: NodeProps<ChatNodeData>) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const historyContents = ancestorChain.flatMap(node => {
         const parts = [];
         // User turn
         if (node.inputText) {
         ...
         // Model turn
         if (node.aiResponse) {
             parts.push({ role: 'model', parts: [{ text: node.aiResponse as string }] });
         }
         return parts;
      });
      ...
      const streamResult = await ai.models.generateContentStream({
        ...
        contents: contents,
        config: {
          systemInstruction: "You are a helpful assistant. Please keep your responses concise, short, and to the point. Use Markdown formatting where appropriate.",
        }
      });
      ...
      setResponse("Error generating response. Please check if your API key is configured correctly.");
    } finally {
  ...
  return (
    <div 
      ref={nodeRef}
      className="bg-white rounded-xl shadow-lg border border-slate-200 w-[550px] overflow-hidden flex flex-col transition-shadow duration-200 hover:shadow-xl relative group"
    >
      ...
      {(response || isGenerating) && (
        <div className="border-t border-slate-100 bg-slate-50 p-4 relative">
            <div 
                ref={responseRef}
                className={`text-sm leading-relaxed text-slate-700 select-text cursor-text nodrag transition-all duration-300 ${
                  isResponseCollapsed ? 'max-h-72 overflow-hidden' : ''
                }`}
            >
